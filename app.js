...
list.innerHTML = "";
  ...
  resources.forEach((r, i) => {
    ...
    title.onclick = () => window.open(r.url, "_blank");
...
function endDrag() {
  ...
  // 1. Map out the newly ordered visible projects based on their old index
  const reorderedVisibleProjects = newCards.map(card => {
    const index = Number(card.dataset.index);
    return projects[index];
  });

  // 2. Reconstruct the array: swap visible items into their new slots, keep hidden items in place
  let visibleCounter = 0;
  const newProjectsArray = projects.map(p => {
    const isVisible = showArchived ? p.archived : !p.archived;
    
    if (isVisible) {
      return reorderedVisibleProjects[visibleCounter++];
    } else {
      return p; 
    }
  });

  // 3. Update the main state safely
